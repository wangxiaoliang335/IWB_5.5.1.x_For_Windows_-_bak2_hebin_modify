...
void CGestureEventGenerator::CancelMouseButtonUp(const TLightSpot * aryLightSpots,  int nCount)
{
    if (!m_isMouseUp)
    {
        if (nCount > 0)
        {
            GetVirtualMouse().GenEvent(aryLightSpots[0].spotParam.ptPosInScreen, E_LBUTTON_UP);
            m_isMouseUp = true;
        }
    }
}
...
void CGestureEventGenerator::Input2Device(const POINT *ptArry, const int &nActualCount, bool needMatch)
{
    ...
    refVirtualHid.InputPoints(penInfo, nElementCount, NULL, 0);
}
...
{       
    ...
    do 
    {
      ...
      m_hwnd = FindWindow(NULL, _T("tBoard"));
        ...
        if (!PtInRect(&rc, aryLightSpots[0].spotParam.ptPosInScreen)) {bRet = FALSE; break;}
    ...
    else
    {
        ...
        else
        {
            ...
            else
            {
                ...
                for(int i = 0; i < nCount; i++)
                {
                    pts[i] = aryLightSpots[i].spotParam.ptPosInScreen;
                }
                nActualCount = nCount;
...
BOOL  CGestureEventGenerator::EraserTransPen(const TLightSpot * aryLightSpots,  int nCount)
{
    ...
    POINT ptEraser = aryLightSpots[0].spotParam.ptPosInScreen ;
    bool isEraser = false;
	...
	case E_WBS_SPOTLIGHT:      //已经判断在聚光灯模式下的操作
		{
            ...
            if(m_oFingerDoubleClickChecker.FingerDoubleClick(aryLightSpots[0].spotParam.ptPosInScreen))
            {
	...
	case E_WBS_ERASER:               //已经判断在板擦的模式下操作
		{
            ...
            if(/*nSumArea > aryLightSpots[0].lStdSpotAreaInVideo * m_nMultiEraser*/isEraser)
            {
                ...
                if (m_oLongTimeClickTrigger.DetectLongTimeStay(aryLightSpots[0].spotParam.ptPosInScreen))
                {
                    //聚光灯
                    PostMessage(m_hwnd,MyWM_SpotLight,aryLightSpots[0].spotParam.ptPosInScreen.x ,aryLightSpots[0].spotParam.ptPosInScreen.y );
  ...
  //          // }
	...
	case E_WBS_SELECTION:                            //已经判断为选择对象的模式了。
		{            
            ...
            for (int i = 0; i < nCount; i++)
            {
                if (abs(aryLightSpots[i].spotParam.ptPosInScreen.x - m_ptResponse.x) < 100 && abs(aryLightSpots[i].spotParam.ptPosInScreen.y - m_ptResponse.y) < 100)
                {//剔除干扰点（与上一次平均值相差过大的点）
                    xSum += aryLightSpots[i].spotParam.ptPosInScreen.x;
                    ySum += aryLightSpots[i].spotParam.ptPosInScreen.y;
                }
            }
    ...
    //                    
				...
				if (m_eRecordState == E_START)
				{
                    ...
                    //原因：当手掌处于白板边缘位置擦处时，误触发双击事件
                      if (aryLightSpots[0].spotParam.ptPosInScreen.x > g_oWGRConfig.MapStandardX2ActualValue(70)  && aryLightSpots[0].spotParam.ptPosInScreen.x < m_nScreenX - g_oWGRConfig.MapStandardX2ActualValue(80))
                      {//白板左右边缘，不触发双击事件
                          //判断是不是双击产生的效果，如果是双击产生的话，说明是返回到桌面的情况。
                          if (m_oHandDoubleClickChecker.HandDoubleClick(aryLightSpots[0].spotParam.ptPosInScreen))
                          {
				...
				else
				{
					...
					{
						m_ptInputCoord = aryLightSpots[0].spotParam.ptPosInScreen ;
						m_CollectCount++ ;
					}
					else
					{
					...
					case E_COLLECT_START:
						{
							...
							if(aryLightSpots[0].spotParam.ptPosInScreen.x < aryLightSpots[1].spotParam.ptPosInScreen.x)
							{
								m_Start_MousePoint[0] = aryLightSpots[0].spotParam.ptPosInScreen  ;
								m_Start_MousePoint[1] = aryLightSpots[1].spotParam.ptPosInScreen  ;
							}
							else
							{
								m_Start_MousePoint[0] = aryLightSpots[1].spotParam.ptPosInScreen  ;
								m_Start_MousePoint[1] = aryLightSpots[0].spotParam.ptPosInScreen  ;
							}
					...
					case E_COLLECT_CONTINUE:
						{
							...
							long dx0 = (aryLightSpots[0].spotParam.ptPosInScreen.x - m_Last_Multipoint[0].x ) ;
							long dy0 = (aryLightSpots[0].spotParam.ptPosInScreen.y - m_Last_Multipoint[0].y ) ;
							long dx1 = (aryLightSpots[0].spotParam.ptPosInScreen.x - m_Last_Multipoint[1].x ) ;
							long dy1 = (aryLightSpots[0].spotParam.ptPosInScreen.y - m_Last_Multipoint[1].y ) ;

							if ( dx0 *dx0 + dy0 * dy0  <dx1*dx1 +dy1*dy1 )
							{
								m_MultiPoint[0] = aryLightSpots[0].spotParam.ptPosInScreen   ;
								m_MultiPoint[1] = aryLightSpots[1].spotParam.ptPosInScreen   ;
							}
							else
							{
								m_MultiPoint[0] = aryLightSpots[1].spotParam.ptPosInScreen   ;
								m_MultiPoint[1] = aryLightSpots[0].spotParam.ptPosInScreen   ;
							}
...
// 							double  cos120_squre  =  0.25 * double(V0_square) * double(V1_square) ;
 ...
 //                           	g_oMouseEventGen.SetSimulMouseResponseState(CMouseEventGenerator::SIMUL_MOUSE_STATE_IDLE) ; 
							...
							else
							{
								...
								POINT Max_pt = aryLightSpots[0].spotParam.ptPosInScreen ;
								POINT Sec_pt = aryLightSpots[1].spotParam.ptPosInScreen ;
								...
								if (abs(Max_pt.x - Sec_pt.x) > nXRoamLimit || abs(Max_pt.y - Sec_pt.y) > nYRoamLimit )
								{
									...
									if (m_CollectCount == 1)
									{
										...
										///鼠标跟随X坐标最小的值来进行判断
										POINT ptRoam = aryLightSpots[0].spotParam.ptPosInScreen   ;

										for (int i =1 ;i < nCount ; i++ )
										{
											if (ptRoam.x >= aryLightSpots[i].spotParam.ptPosInScreen.x)
											{
												ptRoam = aryLightSpots[i].spotParam.ptPosInScreen    ;
											}
										}
        ...
        //                                /*AtlTrace(_T("*******************Stroke Selection Begin : \r\n"));*/
				...
				//if (pptMouse)
				{
					m_ptResponse =aryLightSpots[0].spotParam.ptPosInScreen ;
				}
...
BOOL CGestureEventGenerator::DetectMouseWheelAction(TLightSpot * aryLightSpots, int nCount)
{
	...
	case E_GESTUREEVENT_START_COLLECT:
		{
			for (int i =0 ; i< nCount ; i++ )
			{
                m_MultiPoint[i] = aryLightSpots[i].spotParam.ptPosInScreen ;
	...
	case E_GESTUREEVENT_CONTINUE_COLLECT:
	{
        ...
        if (abs(aryLightSpots[0].spotParam.ptPosInScreen.x - m_Start_MousePoint[0].x) < abs(aryLightSpots[0].spotParam.ptPosInScreen.x - m_Start_MousePoint[1].x) )
		{
			m_MultiPoint[0]  = aryLightSpots[0].spotParam.ptPosInScreen;
			m_MultiPoint[1]  = aryLightSpots[1].spotParam.ptPosInScreen;
...
bool CGestureEventGenerator::Match2Touchs(const TLightSpot * aryLightSpots,  int nCount)
{
    ...
    */
    ...
    long dx = aryLightSpots[0].spotParam.ptPosInScreen.x - m_ptResponse.x;
    long dy = aryLightSpots[0].spotParam.ptPosInScreen.y - m_ptResponse.y;
    ...
    for (int i = 1; i < nCount; i++)
    {
        dx = aryLightSpots[i].spotParam.ptPosInScreen.x - m_ptResponse.x;
        dy = aryLightSpots[i].spotParam.ptPosInScreen.y - m_ptResponse.y;
    ...
    {
        //
        m_ptResponse = aryLightSpots[nMatchPos].spotParam.ptPosInScreen;
...
bool CGestureEventGenerator::LeastDistanceMatch(const TLightSpot * aryLightSpots, int nCount, POINT &pt)
{
    ...
    long dx0 = (aryLightSpots[0].spotParam.ptPosInScreen.x - pt.x ) ;
    long dy0 = (aryLightSpots[0].spotParam.ptPosInScreen.y - pt.y ) ;
    long dx1 = (aryLightSpots[1].spotParam.ptPosInScreen.x - pt.x ) ;
    long dy1 = (aryLightSpots[1].spotParam.ptPosInScreen.y - pt.y ) ;
    ...
    if(lDistSquare0 < lDistSquare1)
    {
        pt = aryLightSpots[0].spotParam.ptPosInScreen;
    }
    else
    {
        pt = aryLightSpots[1].spotParam.ptPosInScreen;
    }
