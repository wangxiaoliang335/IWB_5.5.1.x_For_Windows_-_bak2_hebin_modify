...
CIWBSensor::CIWBSensor(int nID)
    ...
    m_oShootCalibrator(&m_oPenPosDetector)
...
void CIWBSensor::Reset_Start_Offset()
{
    this->m_oPenPosDetector.Reset_Start_Offset();
}
...
void CIWBSensor::OnAutoCalibrateDone(BOOL bSuccess)
{
    ...
    else//成功
    {
        ...
        m_oVideoPlayer.SetDisplayInfo(g_oResStr[IDS_STRING481], 300, FALSE);
...
inline void FillCircle(HWND hWnd, COLORREF clr, const RECT* pRect = NULL)
{
    HDC hDC = ::GetDC(hWnd);
    HBRUSH hBrush = ::CreateSolidBrush(clr);//
    RECT rcClient;
...
//      nPtsInCol, 每列的校正点个数
//      hNotifyWnd, 校正结束后的同志消息的接收窗体
void  CIWBSensor::StartShootCalibrate(HWND hNotifyWnd)
{
    ...
    rcCircle.left = (rcVirtualScreen.right - rcVirtualScreen.left)/2 - 40;
    rcCircle.top = (rcVirtualScreen.bottom - rcVirtualScreen.top) / 2 - 40;
    rcCircle.right = (rcVirtualScreen.right - rcVirtualScreen.left) / 2 + 40;
    rcCircle.bottom = (rcVirtualScreen.bottom - rcVirtualScreen.top) / 2 + 40;
    ...
    wsprintf(szTmp, L" SM_CXVIRTUALSCREEN:%d, SM_CYVIRTUALSCREEN:%d, SM_CXSCREEN:%d, SM_CYSCREEN:%d\n", 
        GetSystemMetrics(SM_CXVIRTUALSCREEN), GetSystemMetrics(SM_CYVIRTUALSCREEN), GetSystemMetrics(SM_CXSCREEN),
        GetSystemMetrics(SM_CYSCREEN));
    OutputDebugString(szTmp);

    //生成校正窗体，尺寸为包含所有屏幕的虚拟屏幕的尺寸
    if (HWND(m_oShootCalibrator) == NULL)
    {
        if (!(m_oShootCalibrator.CreateWindow)(rcVirtualScreen))
        {
            return;
        }
    }
    ...
    {   //在更改分屏幕辨率时，需要调整校正窗体的尺寸
        m_oShootCalibrator.MoveWindow(rcVirtualScreen);
    }
    ...
    m_oPenPosDetector.ResetWaitTimer();
    /**************************************************/

    FillBoard(m_oShootCalibrator, BACKGROUND_COLOR, &rcVirtualScreen);

    FillCircle(m_oShootCalibrator, RGB(255, 0, 0), &rcCircle);

    FillCircle(m_oShootCalibrator, RGB(255, 255, 255), &rcSubCircle);

    //m_eCalibrateError = E_AUTO_CALIBRATOR_ERROR_UNDEF;
    //m_hNotifyWnd = autoCalibrateParams.hNotifyWnd;
    //m_oShootCalibrator.ShowWindow(SW_SHOW);
    //m_oShootCalibrator.SetFocus();

    SwitchLensMode(E_NORMAL_USAGE_MODE);

    m_oPenPosDetector.EntryShootCalibrateMode(m_oShootCalibrator/*m_oAutoCalibrator.GetCalibrateHWnd()*/, TRUE, m_oShootCalibrator.GetCloseBtnRect());
...
void CIWBSensor::LeaveShootCalibrateMode()
{
