...
BOOL LoadConfig(TiXmlNode *pNode, TAdvancedSettings& advanceSettings)
{
    ...
    do
    {
        ...
        if (_stricmp(lpszElementName, "Param") == 0)
        {
            ...
            else if (_stricmp(paramName, "ShootOffsetX") == 0)
            {
                sendParams[0] = atoi(paramValue);
            }
            else if (_stricmp(paramName, "ShootOffsetY") == 0)
            {
                sendParams[1] = atoi(paramValue);
            }
        }
...
BOOL SaveConfig(TiXmlNode *pNode, const TAdvancedSettings& advanceSettings)
{
    ...
    //屏幕物理宽度
    pXmlComment = new TiXmlComment("射击偏移X，单位:英寸");
    pNode->LinkEndChild(pXmlComment);

    pElement = new TiXmlElement("Param");
    pElement->SetAttribute("name", "ShootOffsetX");
    pElement->SetAttribute("value", sendParams[0]);
    pNode->LinkEndChild(pElement);


    //屏幕物理高度
    pXmlComment = new TiXmlComment("射击偏移Y，单位:英寸");
    pNode->LinkEndChild(pXmlComment);

    pElement = new TiXmlElement("Param");
    pElement->SetAttribute("name", "ShootOffsetY");
    pElement->SetAttribute("value", sendParams[1]);
    pNode->LinkEndChild(pElement);
