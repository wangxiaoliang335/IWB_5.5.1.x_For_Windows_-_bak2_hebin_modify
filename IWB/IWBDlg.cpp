...
int sendParams[2] = { 0, 0 };
//volatile bool bStart_Offset = false;
...
BEGIN_MESSAGE_MAP(CIWBDlg, CDialog)
    ...
    ON_COMMAND(ID_MENU_CIRCLE_SCREEN_SHOOT_CALIBRATE, &CIWBDlg::OnMenuShootCalibrate)
...
//ON_MESSAGE(WM_DISPLAYWINDOW,&CIWBDlg::OnDisPlayWindow)
  ...
  //  ON_COMMAND_RANGE(ID_SENSORCTXMENU_RUN,ID_SENSORCTXMENU_FOURPOINTCALIBRATION, OnSensorCtxMenu)
    ...
    ON_COMMAND(ID_CS_GAME_PARAMETER, &CIWBDlg::OnMenuCSGameParamter)
...
void CIWBDlg::InitMenu()
{
    ...
    //m_oMenu.RemoveMenu(ID_MENU_CIRCLE_SCREEN_SHOOT_CALIBRATE, MF_BYCOMMAND);  //wxl modify 删除射击校正菜单
...
struct recvThreadPara {
    HANDLE hPipe;
    CIWBDlg* pThis;
};

recvThreadPara g_recvThreadPara;

#define PIPE_NAME L"\\\\.\\pipe\\MyIntPipe"
...
DWORD WINAPI HandleClient(LPVOID lpParam) {
    ...
    HANDLE hPipe = ((recvThreadPara*)(lpParam))->hPipe;
    CIWBDlg* pThis = ((recvThreadPara*)(lpParam))->pThis;
    int receivedParams[2];
    DWORD dwRead;
    DWORD dwWritten;

    // 读取客户端发送的两个int参数
    if (ReadFile(hPipe, receivedParams, sizeof(receivedParams), &dwRead, NULL) != FALSE) {
        std::cout << "收到客户端参数: " << receivedParams[0] << ", " << receivedParams[1] << std::endl;

        wchar_t szTmp[1024] = { 0 };
        wsprintf(szTmp, L" receivedParams 0:%d, 1:%d\n", receivedParams[0], receivedParams[1]);
        OutputDebugString(szTmp);
        if (receivedParams[0] == 1 && receivedParams[1] == 1)
        {
            pThis->StartShootCalibrate();
        }
        else if (receivedParams[0] == 0 && receivedParams[1] == 0) {
            pThis->LeaveShootCalibrate();
        }

        //// 向客户端发送两个int参数
        //if (WriteFile(hPipe, sendParams, sizeof(sendParams), &dwWritten, NULL) != FALSE) {
        //    std::cout << "已向客户端发送参数: " << sendParams[0] << ", " << sendParams[1] << std::endl;
        //}
        //else {
        //    std::cerr << "向客户端发送数据失败，错误代码: " << GetLastError() << std::endl;
        //}
    }
    else {
        std::cerr << "读取客户端数据失败，错误代码: " << GetLastError() << std::endl;
    }
...
DWORD WINAPI ServerThread(LPVOID lpParam) {
    ...
    if (hPipe == INVALID_HANDLE_VALUE) {
        std::cerr << "创建命名管道失败，错误代码: " << GetLastError() << std::endl;
        return 1;
    }
    ...
    while (!g_bExit) {
        ...
        if (ConnectNamedPipe(hPipe, NULL) != FALSE) {
            ...
            g_recvThreadPara.hPipe = hPipe;
            g_recvThreadPara.pThis = pThis;
            ...
            if (hPipe == INVALID_HANDLE_VALUE) {
                std::cerr << "创建新的命名管道实例失败，错误代码: " << GetLastError() << std::endl;
                break;
            }
        }
        else {
            if (GetLastError() == ERROR_OPERATION_ABORTED) {
                std::cout << "ConnectNamedPipe操作被取消" << std::endl;
            }
            else {
                std::cerr << "客户端连接失败，错误代码: " << GetLastError() << std::endl;
            }
        }
    }
...
//
//const char* AES_KEY1 = "01234567890123456789012345678901";
//const char* AES_IV = "0123456789012345";
//
...
//            EVP_DecryptInit_ex(ctx, EVP_aes_256_cbc(), NULL, (unsigned char*)AES_KEY1, (unsigned char*)AES_IV);
//
//            int plaintext_len;
//            unsigned char plaintext[BUFFER_SIZE];
//            int len1, len2;
//            EVP_DecryptUpdate(ctx, plaintext, &len1, (unsigned char*)buffer, len);
//            EVP_DecryptFinal_ex(ctx, plaintext + len1, &len2);
//            plaintext_len = len1 + len2;
//            EVP_CIPHER_CTX_free(ctx);
//
//            plaintext[plaintext_len] = '\0';
//            std::cout << "Received: " << plaintext << std::endl;
//
//            // 回应同样的内容（加密后发送）
//            EVP_CIPHER_CTX* ctx2 = EVP_CIPHER_CTX_new();
//            unsigned char ciphertext[BUFFER_SIZE];
//            int clen1, clen2;
//            EVP_EncryptInit_ex(ctx2, EVP_aes_256_cbc(), NULL, (unsigned char*)AES_KEY1, (unsigned char*)AES_IV);
//            EVP_EncryptUpdate(ctx2, ciphertext, &clen1, plaintext, plaintext_len);
//            EVP_EncryptFinal_ex(ctx2, ciphertext + clen1, &clen2);
//            int total_len = clen1 + clen2;
//            EVP_CIPHER_CTX_free(ctx2);
//
//            send(client_sock, (char*)ciphertext, total_len, 0);
//        }
//    }
//
...
//void handle_server(SOCKET client_sock) {
//    WSADATA wsaData;
...
const char* AES_KEY1 = "01234567890123456789012345678901"; // 32 字节密钥
const char* AES_IV = "0123456789012345";                   // 16 字节 IV

std::atomic<bool> bStop = false;

// 解密函数
int aes_decrypt(const unsigned char* ciphertext, int len, unsigned char* plaintext) {
    EVP_CIPHER_CTX* ctx = EVP_CIPHER_CTX_new();
    int len1 = 0, len2 = 0;
    EVP_DecryptInit_ex(ctx, EVP_aes_256_cbc(), NULL, (unsigned char*)AES_KEY1, (unsigned char*)AES_IV);
    EVP_DecryptUpdate(ctx, plaintext, &len1, ciphertext, len);
    EVP_DecryptFinal_ex(ctx, plaintext + len1, &len2);
    EVP_CIPHER_CTX_free(ctx);
    return len1 + len2;
}
...
// 加密函数
int aes_encrypt(const unsigned char* plaintext, int len, unsigned char* ciphertext) {
    EVP_CIPHER_CTX* ctx = EVP_CIPHER_CTX_new();
    int len1 = 0, len2 = 0;
    EVP_EncryptInit_ex(ctx, EVP_aes_256_cbc(), NULL, (unsigned char*)AES_KEY1, (unsigned char*)AES_IV);
...
void handle_client(SOCKET client_sock) {
    ...
    while (!bStop) {
        ...
        int decrypted_len = aes_decrypt((unsigned char*)buffer, len, decrypted);
        ...
        int encrypted_len = aes_encrypt((const unsigned char*)szUSBBuffer, strlen(szUSBBuffer)/*decrypted_len*/, encrypted);
        send(client_sock, (char*)encrypted, encrypted_len, 0);
    }
...
void network_thread() {
    ...
    while (!bStop) {
        ...
        timeval timeout = { 0, 0 }; // 非阻塞 select（立即返回）
...
BOOL CIWBDlg::OnInitDialog()
{
    ...
    if (hServerThread == NULL) {
        std::cerr << "创建服务端线程失败，错误代码: " << GetLastError() << std::endl;
        return 1;
    }

    // 这里主线程可以处理其他事务
    std::cout << "主线程可以处理其他任务..." << std::endl;
    ...
    net_thread = std::thread(network_thread);

    //ID_MENU_CIRCLE_SCREEN_MANUAL_CALIBRATE
        // 获取对话框的菜单句柄
    HMENU hMenu = GetMenu()->GetSafeHmenu();
    if (hMenu == NULL)
        return TRUE;

    // 方法1：使用EnableMenuItem隐藏（推荐）
    // 参数：菜单项ID，MF_BYCOMMAND表示按ID查找，MF_GRAYED表示灰显并禁用
    // 注意：MF_GRAYED会同时禁用菜单项，视觉上表现为隐藏（某些系统主题下）
    //EnableMenuItem(hMenu, ID_FILE_EXIT, MF_BYCOMMAND | MF_GRAYED);

    // 方法2：使用ModifyMenu完全移除（不推荐，难以恢复）
    // ModifyMenu(hMenu, ID_FILE_EXIT, MF_BYCOMMAND | MF_REMOVE, 0, NULL);

    // 如果是弹出菜单中的项，需要先获取子菜单
    // 例如，隐藏"文件"菜单下的"新建"项（ID_FILE_NEW）
    HMENU hSubMenu = GetSubMenu(hMenu, 0); // 获取第一个子菜单（文件菜单）
    if (hSubMenu)
    {
        EnableMenuItem(hSubMenu, ID_MENU_CIRCLE_SCREEN_MANUAL_CALIBRATE, MF_BYCOMMAND | MF_GRAYED);
    }
...
LRESULT CIWBDlg::OnClose(WPARAM wParam,LPARAM lParam)
{
    ...
    // 取消ConnectNamedPipe操作
    if (hPipe != INVALID_HANDLE_VALUE) {
        if (!CancelIoEx(hPipe, NULL)) {
            std::cerr << "取消I/O操作失败，错误代码: " << GetLastError() << std::endl;
        }
    }

    // 等待服务端线程结束（这里可根据需求修改等待逻辑）
...
void CIWBDlg::OnRawInput(UINT nInputcode, HRAWINPUT hRawInput)
{
    ...
    if (raw->header.dwType == RIM_TYPEKEYBOARD)
    {
        ...
        else if (raw->data.keyboard.VKey == 'G')
        {
            short G_state = GetAsyncKeyState('G');
            short ctrl_state = GetAsyncKeyState(VK_CONTROL);
            short shift_state = GetAsyncKeyState(VK_SHIFT);
            short alt_state = GetAsyncKeyState(VK_MENU);

            if ((G_state & 0x8000) && (ctrl_state & 0x8000) && (/*shift_state & 0x8000 ||*/ alt_state & 0x8000))
            {
                //g_oMouseEventGen.EnableOpticalPenControl(!g_oMouseEventGen.IsOpticalPenControlling());
                OnMenuShootCalibrate();
            }
        }
    }
...
void CIWBDlg::StartShootCalibrate()
{
    OutputDebugString(L"Entry StartShootCalibrate\n");
    OnMenuShootCalibrate();
}

void CIWBDlg::LeaveShootCalibrate()
{
    OutputDebugString(L"Entry LeaveShootCalibrate\n");
    LeaveShootCalibrateMode();
}
...
void CIWBDlg::OnMenuShootCalibrate()
{
    OutputDebugString(L"...... 开始射击校正\n");
    sendParams[0] = 0;
    sendParams[1] = 0;
    //bStart_Offset = false;
    this->m_oIWBSensorManager.StartShootCalibrate(this->GetSafeHwnd(), -1, -1, -1);
}

void CIWBDlg::LeaveShootCalibrateMode()
{
    OutputDebugString(L"...... 停止射击校正\n");
    this->m_oIWBSensorManager.LeaveShootCalibrateMode(-1);
}
...
{
    // TODO: Add your message handler code here and/or call default
    OutputDebugString(L" CIWBDlg OnNcLButtonDblClk");
...
void CIWBDlg::OnMenuCSGameParamter()
{
    CDlgCSGameParameter  dlgcsGamePara;
    if (dlgcsGamePara.DoModal())
    {
...
void CIWBDlg::OnMenuAdvancessetting()
{
    ...
    CameraFmtDialog  camerafmtdlg(m_oIWBSensorManager.GetSpotListProcessor().GetVirtualHID().IsVirtualDriverOpen(),
        m_oIWBSensorManager.GetSpotListProcessor().GetVirtualHID().GetHIDMode(),
        m_oIWBSensorManager.GetSpotListProcessor().GetVirtualHID().GetTouchHIDMode(),
        m_oIWBSensorManager.GetSpotListProcessor().GetVirtualHID().GetTouchTUIOMode());
    ...
    //camerafmtdlg.InitDeviceUseModeExtern(m_oIWBSensorManager.GetSpotListProcessor().GetVirtualHID().IsVirtualDriverOpen(),
    //    m_oIWBSensorManager.GetSpotListProcessor().GetVirtualHID().GetHIDMode(),
    //    m_oIWBSensorManager.GetSpotListProcessor().GetVirtualHID().GetTouchHIDMode(),
    //    m_oIWBSensorManager.GetSpotListProcessor().GetVirtualHID().GetTouchTUIOMode());
    ///////////////////////////////////
    ...
    if (camerafmtdlg.DoModal() == IDOK)
    {
        ...
        if (TRUE == bHIDMode)
        {
            if (E_DEV_MODE_MOUSE == camerafmtdlg.GetDeviceMode())
            {
                m_oIWBSensorManager.GetSpotListProcessor().GetVirtualHID().SetHIDMode(E_DEV_MODE_MOUSE);
