...
//        -4,软件狗硬件ID错误
//        -5,未使能
void errCodeConvertString(int code, wchar_t *StrErr)
{
    if (0 == code)
    {
        wsprintf(StrErr, L"%s", g_oResStr[IDS_STRING500]);
    }
    else if (-1 == code)
    {
        wsprintf(StrErr, L"%s", g_oResStr[IDS_STRING501]);
    }
    else  if (-2 == code)
    {
        wsprintf(StrErr, L"%s", g_oResStr[IDS_STRING502]);
    }
    else  if (-3 == code)
    {
        wsprintf(StrErr, L"%s", g_oResStr[IDS_STRING503]);
    }
    else  if (-4 == code)
    {
        wsprintf(StrErr, L"%s", g_oResStr[IDS_STRING504]);
    }
    else  if (-5 == code)
    {
        wsprintf(StrErr, L"%s", g_oResStr[IDS_STRING505]);
    }
    return;
}

// CIWBApp initialization
char szUSBBuffer[1024] = { 0 };
wchar_t szUSBRegBuffer[1024] = { 0 };
HRESULT g_hr13 = S_OK;
BOOL CIWBApp::InitInstance()
{
    ...
    BOOL bCheckUSBKey = ReadUSBKey(bFirstTime);
    if (!bCheckUSBKey)
    {
        return FALSE;
    }
    ...
    /**********************************************/
    HRESULT hr1 = SDKREG_IsBasicFunctionEnabled(1);
    HRESULT hr2 = SDKREG_IsBasicFunctionEnabled(2);
    HRESULT hr3 = SDKREG_IsBasicFunctionEnabled(3);
    HRESULT hr4 = SDKREG_IsBasicFunctionEnabled(4);
    HRESULT hr5 = SDKREG_IsBasicFunctionEnabled(5);
    HRESULT hr6 = SDKREG_IsBasicFunctionEnabled(6);
    HRESULT hr7 = SDKREG_IsBasicFunctionEnabled(7);
    HRESULT hr8 = SDKREG_IsBasicFunctionEnabled(8);
    HRESULT hr9 = SDKREG_IsBasicFunctionEnabled(9);
    HRESULT hr10 = SDKREG_IsBasicFunctionEnabled(10);
    HRESULT hr11 = SDKREG_IsBasicFunctionEnabled(11);
    HRESULT hr12 = SDKREG_IsBasicFunctionEnabled(12);
    HRESULT hr13 = SDKREG_IsBasicFunctionEnabled(13);
    HRESULT hr14 = SDKREG_IsBasicFunctionEnabled(14);
    HRESULT hr15 = SDKREG_IsBasicFunctionEnabled(15);
    ...
    sprintf(szUSBBuffer, " BasicFunction hr1:%d,hr2:%d,hr3:%d,hr4:%d,hr5:%d,hr6:%d,hr7:%d,hr8:%d,hr9:%d,hr10:%d,hr11:%d,hr12:%d,hr13:%d,hr14:%d,hr15:%d\n",
        hr1,
        hr2, 
        hr3, 
        hr4, 
        ...
        hr14, 
        hr15);
    OutputDebugStringA(szUSBBuffer);

    wchar_t szHrStr1[1024] = { 0 };
    errCodeConvertString(hr1, szHrStr1);

    wchar_t szHrStr2[1024] = { 0 };
    errCodeConvertString(hr2, szHrStr2);

    wchar_t szHrStr13[1024] = { 0 };
    errCodeConvertString(hr13, szHrStr13);

    wchar_t szHrStr14[1024] = { 0 };
    errCodeConvertString(hr14, szHrStr14);
    wsprintf(szUSBRegBuffer, L"%s1:%s\n%s2:%s\n%s13:%s\n%s14:%s",
        g_oResStr[IDS_STRING506],
        szHrStr1,
        g_oResStr[IDS_STRING506],
        szHrStr2,
        g_oResStr[IDS_STRING506],
        ...
        g_oResStr[IDS_STRING506],
        szHrStr14);
    OutputDebugString(szUSBRegBuffer);
    /**********************************************/
...
//      第二次及以后则不再弹出对话框。
BOOL CIWBApp::ReadUSBKey(BOOL bFirstTime)
{
