...
public:
	...
	void LoadConfig(BOOL isHighDefinition);
    ...
    BOOL DoDetect(const CImageFrame* pGrayFrame, ESensorLensMode eSensorLenMode, map<int, pointRect>& nID_Para, CAtlString strFullPath, int index);
...
//	void UpdateManualScreenAreaMask();
	...
	void EntryShootCalibrateMode(HWND hCalibrateWnd, int isShootCalibrate, CRect closeRect);
...
//	RectF  m_rcGuideRel          ;//相对的引导矩形框的区域
   ...
   //<<debug
	...
	int m_frameCount[5] = { 0, 0, 0, 0, 0 };
	map<int, int>  m_IndexToHaveSpot[5];

	//多个枪的情况，最多五把枪
	pointRect  spot_Point_[5];

	map<int, int> m_mapSpotPointToIndex;

	static const  double SCREEN_DISTANCETWOSPOT_WIDTH_IN_MM;
	double m_ScreenMinDistanceWidthInPixel ;

	TLightSpot aryLightSpotsID[5];

	std::thread  net_client_thread;

	map<int, TLightSpot> m_gunIdToLightSpot;

public:
	std::queue<string> coordQueueInfo;
	std::mutex mtx;

private:
	char szCoordTmp[1024] = { 0 };
