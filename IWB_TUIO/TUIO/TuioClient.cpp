	...
	try {
		...
		if( strcmp( msg.AddressPattern(), "/tuio/2Dobj" ) == 0 ){
			...
			if (strcmp(cmd,"source")==0) {
                ...
                char szBuf[1024] = { 0 };
                sprintf(szBuf, "111 22 TuioClient s_id:%d, maccel:%d, iCameraId:%d, iGunid:%d\n", s_id, maccel, iCameraId, iGunid);
				...
				if (tcur==cursorList.end()) {
					...
					TuioCursor *addCursor = new TuioCursor((long)s_id,-1,xpos,ypos, iCameraId, iGunid);
					...
					TuioCursor *updateCursor = new TuioCursor((long)s_id,(*tcur)->getCursorID(),xpos,ypos, iCameraId, iGunid);
					...
					for (std::list<TuioCursor*>::iterator iter=frameCursors.begin(); iter != frameCursors.end(); iter++) {
						...
						switch (tcur->getTuioState()) {
							...
							case TUIO_ADDED:
								...
								frameCursor = new TuioCursor(currentTime,tcur->getSessionID(),c_id,tcur->getX(),tcur->getY(), tcur->getCameraId(), tcur->getGunId());
