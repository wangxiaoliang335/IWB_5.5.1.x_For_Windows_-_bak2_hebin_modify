...
TuioCursor* TuioManager::addTuioCursor(float x, float y, DWORD dwCameraId, int gunId) {
	sessionID++;
	...
	TuioCursor *tcur = new TuioCursor(currentFrameTime, sessionID, cursorID, x, y, dwCameraId, gunId);
...
void TuioManager::removeTuioCursor()
{
	for (auto it = cursorList.begin(); it != cursorList.end(); ) {
		auto itT = it;
		delete* itT;
		it = cursorList.erase(it);
	}
}
