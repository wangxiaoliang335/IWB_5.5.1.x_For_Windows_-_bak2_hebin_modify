...
TuioServer::TuioServer(bool bAnthoer)
	:local_sender			(true)
	,full_update			(false)	
...
{
	m_bAnthoer = bAnthoer;
	primary_sender = new UdpSender(bAnthoer);
	initialize();
}
...
void TuioServer::addCursorMessage(TuioCursor *tcur) {
    ...
    char szBuf[1024] = { 0 };
    sprintf(szBuf, "111 11 addCursorMessage tcur sessionID:%d, xpos:%f, ypos:%f, xvel:%f, yvel:%f, MotionAccel:%f, CameraId:%d, gunId:%d, m_bAnthoer:%d\n", 
		(int32)(tcur->getSessionID()), xpos, ypos, xvel, yvel, tcur->getMotionAccel(), (int32)tcur->getCameraId(), (int32)tcur->getGunId(), m_bAnthoer);
    OutputDebugStringA(szBuf);
	...
	//if (S_OK == g_hr13)
	{
		...
		(*oscPacket) << xvel << yvel << tcur->getMotionAccel() << (int32)tcur->getCameraId() << (int32)tcur->getGunId();  // wxl modify
		(*oscPacket) << osc::EndMessage;
	}
	//else
	//{
